...
if (!this.addedSkillTypes().contains(skill.stypeId)) { return false; }
...
SkillShopSkills.prototype.itemRect = function(index) {
    ...
    if (this._confirmMode && this._confirmSkill) {

        this.changeTextColor(this.systemColor());

this.drawText(
    "Learn " + this._confirmSkill.name + "?",
    0,
    0,
    this.contentsWidth(),
    "center"
...
SkillShopSkills.prototype.startConfirm = function(skill) {
    this._savedIndex = this.index();

    this._confirmMode = true;
    this._confirmSkill = skill;

    this.select(0);
    this.refresh();
};

SkillShopSkills.prototype.endConfirm = function() {
    this._confirmMode = false;
    this._confirmSkill = null;

    this.refresh();

    if (this._savedIndex !== undefined) {
        this.select(this._savedIndex);
        this._savedIndex = undefined;
    }
};

SkillShopSkills.prototype.isConfirmMode = function() {
...
this._createCostWindow();
    ...
    if (this._skillsWindow.isConfirmMode()) {

        if (this._skillsWindow.index() === 0) {

            this._pendingActor.buySkill(
                this._pendingSkill
            );

            this._skillsWindow.endConfirm();
    ...
    this._pendingActor =
        this._skillsWindow.actor;

    this._pendingSkill =
        this._skillsWindow.skill();

    this._skillsWindow.startConfirm(
        this._pendingSkill
    );
