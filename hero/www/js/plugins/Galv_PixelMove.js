...
// Saves made before Pixel Move do not contain this property. Restore a safe
// centred state on demand instead of assuming every loaded character has it.
Game_CharacterBase.prototype.ensureQuadrant = function() {
	if (!this._tileQuadrant || typeof this._tileQuadrant.x !== 'number' ||
			typeof this._tileQuadrant.y !== 'number') {
		this.setQuadrant();
	}
};
...
Game_Player.prototype.checkEventTriggerHere = function(triggers) {
	...
	// A below-character event belongs to the player only at the centre of its
	// tile. Pixel Move's rounded coordinate lookup otherwise triggers it while
	// the player is still crossing one of that tile's outer segments.
	if (this._tileQuadrant.x !== 0 || this._tileQuadrant.y !== 0) return;
...
// Set Movement Route is a forced route in RPG Maker MV. Align the player with
// a tile centre before it begins, so every route command starts and ends on
// whole tiles just as it did before Pixel Move was installed.
Galv.PMOVE.Game_Player_forceMoveRoute = Game_Player.prototype.forceMoveRoute;
Game_Player.prototype.forceMoveRoute = function(moveRoute) {
	this.ensureQuadrant();
	if (this._tileQuadrant.x !== 0 || this._tileQuadrant.y !== 0) {
		this.centerTilePos();
	}
	Galv.PMOVE.Game_Player_forceMoveRoute.call(this, moveRoute);
};
...
Game_Player.prototype.moveStraight = function(d) {
	...
	if (this._normMove || this.isMoveRouteForcing()) {
		return Game_CharacterBase.prototype.moveStraight.call(this,d);
	}
    ...
    if (this.isMovementSucceeded()) {
		if (this._followers) this._followers.updateMove();
        this.setDirection(d);
...
// Followers need the same one-third-tile movement as the player. Leaving
// them tile based makes each follow update interpret a fractional target as a
// whole tile and causes the line to stall or overshoot after turns.
Game_Follower.prototype.quadDirX = Game_Player.prototype.quadDirX;
Game_Follower.prototype.quadDirY = Game_Player.prototype.quadDirY;
...
Game_Follower.prototype.moveStraight = function(d) {
	...
	this._tileQuadrant.x = qDirX;
	this._tileQuadrant.y = qDirY;
	this._x = $gameMap.roundXWithDirectionQuad(this._x, d, qDirX);
	this._y = $gameMap.roundYWithDirectionQuad(this._y, d, qDirY);
	this._realX = $gameMap.xWithDirectionQuad(this._x, this.reverseDir(d), qDirX);
	this._realY = $gameMap.yWithDirectionQuad(this._y, this.reverseDir(d), qDirY);
...
Game_Follower.prototype.moveDiagonally = function(horz, vert) {
	...
	this._tileQuadrant.x = qDirX;
	this._tileQuadrant.y = qDirY;
	this._x = $gameMap.roundXWithDirectionQuad(this._x, horz, qDirX);
	this._y = $gameMap.roundYWithDirectionQuad(this._y, vert, qDirY);
	this._realX = $gameMap.xWithDirectionQuad(this._x, this.reverseDir(horz), qDirX);
	this._realY = $gameMap.yWithDirectionQuad(this._y, this.reverseDir(vert), qDirY);
	this._diagDir = Galv.PMOVE.getDir(horz, vert);
	this.setMovementSuccess(true);
...
// When someone joins the party, place the newly visible follower with the
// player. It will form the normal trailing line on the next whole-tile move.
Galv.PMOVE.Game_Follower_refresh = Game_Follower.prototype.refresh;
Game_Follower.prototype.refresh = function() {
	var wasVisible = this._pmoveWasVisible === true;
	Galv.PMOVE.Game_Follower_refresh.call(this);
	var isVisible = this.isVisible();
	if (isVisible && !wasVisible && $gamePlayer) {
		this.locate(Math.round($gamePlayer.x), Math.round($gamePlayer.y));
		this.setDirection($gamePlayer.direction());
		this._pmoveFollowTrail = [];
	}
	if (!isVisible) this._pmoveFollowTrail = [];
	this._pmoveWasVisible = isVisible;
};

// Keep each follower one complete tile (three Pixel Move segments) behind the
// character in front of it. RPG Maker's normal follower logic follows the
// immediate position; with pixel movement that leaves only a one-segment gap.
Galv.PMOVE.Game_Followers_updateMove = Game_Followers.prototype.updateMove;
Game_Followers.prototype.updateMove = function() {
	if (this.areGathering && this.areGathering()) {
		Galv.PMOVE.Game_Followers_updateMove.call(this);
		return;
	}

	for (var i = this._data.length - 1; i >= 0; i--) {
		var follower = this._data[i];
		var leader = i > 0 ? this._data[i - 1] : $gamePlayer;
		var trail = follower._pmoveFollowTrail || (follower._pmoveFollowTrail = []);
		trail.push({ x: leader.x, y: leader.y });

		if (trail.length >= 3) {
			follower.chaseCharacter(trail.shift());
		}
	}
};

Game_Followers.prototype.resetPixelMoveFormation = function() {
	this.forEach(function(follower) {
		// Game_Player.locate() stacks followers on transfers but does not reset
		// Pixel Move's segment state. Keeping the old quadrant makes the rear
		// followers calculate their first new-map steps from stale positions.
		follower.setQuadrant();
		follower._diagDir = false;
		follower._realX = follower._x;
		follower._realY = follower._y;
		follower._pmoveFollowTrail = [];
	});
};
...
Game_Player.prototype.performTransfer = function() {
	...
	if (wasTransferring && this._followers) {
		this._followers.resetPixelMoveFormation();
	}
};
