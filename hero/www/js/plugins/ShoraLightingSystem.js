    ...
    */
    ...
    const TextureManager = {
        ...
        filter: function(options) {
    		...
    		return TextureManager.filteredCache[options.name] = new PIXI.Sprite(renderedTexture);
        }
    };
    ...
    class ShadowManager {
        ...
        scanMapCaster() {
            ...
            // drawing 
            this._upperGraphics.beginFill(lightingManager.data.topBlockAmbient);
        ...
        getShadowGraphics(lighting) {
            const lx = lighting.x + $gameMap.displayX() * $gameMap.tileWidth();
            const ly = lighting.y + $gameMap.displayY() * $gameMap.tileHeight();

            const ox = lx + lighting.config.shadowoffsetx;
            let oy = ly + lighting.config.shadowoffsety;
            ...
            this.graphics.beginFill(lightingManager.data.ambient);
            ...
            this.transform.pivot.set(lx, ly);
            ...
            if (this.haveIgnoreShadow) {
                Graphics.app.renderer.render(this.ignore, this.renderTexture, false, transform);
            }
    ...
    const flicker = {
        update: function(lighting) {
            const flicker = lighting.config.animation.flicker;
            if (!flicker.status) return;
            if (flicker.counter > 0 && Math.randomInt(flicker.counter / 5) !== 0) {
                flicker.counter -= flicker.speed;
                lighting.alpha = flicker.oalpha;
            } else {
                flicker.counter = flicker.max;
                lighting.alpha = flicker.intensity;
            }
        }
    };
    ...
    const offset = {
        update: function(lighting) {
            const x = lighting.config.offset.x, y = lighting.config.offset.y;

            x._changed = false;
            if (x._currentTick <= x._totalTick) {
                x.value = x._original + transition[x.type](x._currentTick / x._totalTick) * x._delta;
                x._currentTick++;
                x._changed = true;
            }

            y._changed = false;
            if (y._currentTick <= y._totalTick) {
                y.value = y._original + transition[y.type](y._currentTick / y._totalTick) * y._delta;
                y._currentTick++;
                y._changed = true;
            }
        }
    ...
    const dest = [3.125, 4.6875, 1.5625, 0]; 
    const angle = {
        update: function(lighting) {
            const angle = lighting.config.angle;
            const character = lighting.character;

            angle._changed = false;
            if (lighting.config.direction && angle.direction != character.direction()) {
                angle.direction = character.direction();
                
                let a1 = dest[angle.direction / 2 - 1];
                if (Math.abs(lighting.rotation - 6.25 - a1) < Math.abs(lighting.rotation - a1)) 
                    lighting.rotation -= 6.25;
                else if (Math.abs(lighting.rotation + 6.25 - a1) < Math.abs(lighting.rotation - a1)) 
                    lighting.rotation += 6.25;

                angle._original = lighting.rotation;
                angle._delta = a1 - angle._original;
                angle._totalTick = 20; angle._currentTick = 0;
                angle.type = 1;

            }

            if (angle._currentTick < angle._totalTick) {
                angle.value = lighting.rotation 
                = angle._original + transition[angle.type](angle._currentTick / angle._totalTick) * angle._delta;
                angle._currentTick++;
                angle._changed = true;
            } 
        }
    ...
    class Lighting extends PIXI.Sprite {
        constructor(config, character) {
            super(PIXI.RenderTexture.create(TextureManager.filter(config).width, TextureManager.filter(config).height));
            this.config = config;
            this.character = character;

            this.anchor.set(0.5);
            this.blendMode = PIXI.BLEND_MODES.ADD;
            this.light = TextureManager.filter(this.config);
            Graphics.app.renderer.render(this.light, this.texture);

            if (this.config.shadow) {
                // TODO: cause issue with scale?
                this.shadowTexture = PIXI.RenderTexture.create(this.width, this.height);
                this.shadow = new PIXI.Sprite(this.shadowTexture);
                shadowManager.calculateRender(this.shadowTexture);
                Graphics.app.renderer.render(this.shadow, this.texture, false);
            }
        ...
        update() {
            ...
            const position_changed = old_x !== this.mapX || old_y !== this.mapY ||  this.config.offset.x._changed || this.config.offset.y._changed;
            this.updateTexture(position_changed);

        }

        updatePosition() {
            this.x = this.character.screenX() + this.config.offset.x.value;
            this.y = this.character.screenY() + this.config.offset.y.value;
            this.mapX = this.character._realX;
            this.mapY = this.character._realY;
        }
        ...
        needRecalculateShadow() {
            return this.offset.x._changed || this.offset.y._changed;
        }

        needRerender() {
            return this.needRecalculateShadow() || this.config.radius._changed || this.config.angle._changed;
        }


        updateTexture(position_changed) {
            if (!this.renderable) return;
            const need_calculate = this.config.shadow && position_changed;
            const need_render = need_calculate || this.config.radius._changed || this.config.angle._changed; 
            
            if (!need_render) return;
            Graphics.app.renderer.render(this.light, this.texture);

            if (need_calculate) {
                this.shadow = shadowManager.getShadowGraphics(this);
            }
    ...
    function registerLight(configJSON) {
        ...
        const radius = Number(configJSON.radius || 100) / 100;
        const angle = (Number(configJSON.angle) || 0) / 57.6;
        const tint = ColorManager.stringToHex(configJSON.tint || 0);
        const offset = {x: Number(configJSON.offset.x) || 0, y: Number(configJSON.offset.y) || 0};
        ...
        const config = {
            ...
            shadowambient: configJSON.shadowambient ? ColorManager.stringToHex(configJSON.shadowambient) : shadowConfig$1.ambient,
            ...
            animation: {
                flicker: {
                    ...
                    intensity: 1 / (1.1 * (Number(configJSON.animation.flicker.flickintensity)) || 1),
                    speed: 20 * (Number(configJSON.animation.flicker.flickspeed) || 0),
                    oalpha: 1,
                    counter: 1000,
                    max: 1000,
                },
    ...
    class LightingManager {
        constructor() {
            this.data = {
                ...
                shadowAmbient: shadowConfig$1.ambient,
                topBlockAmbient: shadowConfig$1.topAmbient,

                softShadow: shadowConfig$1.soft.status,
                softShadowQlt: shadowConfig$1.soft.quality,
                softShadowStr: shadowConfig$1.soft.strength,
            };
        ...
        addLight(config) {
            ...
            this.characterLighting[config.id] = lighting;
        ...
        setLightingState(id, state) {

        }

        setShadowState(id, state) {

        }

        setRadius(id, radius, time, type) {

        }

        setAngle(id, angle, time, type) {
            
        }

        setOffset(id, x, y, time, type) {
            this.setOffsetX(id, x, time, type);
            this.setOffsetY(id, y, time, type);
        }

        setOffsetX(id, newX, time, type) {
            const config = $gameMap._lighting[id];
            const x = config.offset.x;
            x._original = x.value;
            x._delta = newX - x._original;
            x._totalTick = time; x._currentTick = 1;
            if (type) x.type = type - 1;
        }

        setOffsetY(id, newY, time, type) {
            const config = $gameMap._lighting[id];
            const y = config.offset.y;
            y._original = y.value;
            y._delta = newY - y._original;
            y._totalTick = time; y._currentTick = 1;
            if (type) y.type = type - 1;
        }

        setTint(id, tint, time, type) {
    ...
    if (!shadowConfig$1.engineShadow) {
    ...
    const initialize$3 = Game_Character.prototype.initialize;
    Game_Character.prototype.initialize = function() {
        initialize$3.call(this);
        this.lightingParams = {};
    };
    ...
    Game_Character.prototype.updateLighting = function() {
        const cid = this.lightingParams.id;
        if (cid === undefined) {
            if (this.hasLight) {
                cid = this.eventId || 0;
            } else {
        ...
        const hasLighting = !!this.lightingParams.status;
        const isLighting = !!$gameMap._lighting[cid];

        if (hasLighting && !isLighting) {
            lightingManager.addCharacterLighting(this.lightingParams);
        }
        if (!hasLighting && isLighting) {
            lightingManager.removeCharacterLighting(cid);
        }
    };

    const gainItem = Game_Party.prototype.gainItem;
    Game_Party.prototype.gainItem = function() {
        gainItem.apply(this, arguments);
        $gamePlayer.scanItemLighting();
    };

    const refresh = Game_Player.prototype.refresh;
    const initialize$2 = Game_Player.prototype.initialize;

    Game_Player.prototype.initialize = function() {
        initialize$2.apply(this, arguments);
        this.lightingParams.id = 0;
    };
    ...
    Game_Player.prototype.scanLighting = function() {
        ...
        for (const line of note)
            parseNotes(config, line);

        if (config.name) {
            this.lightingParams = config;
        }
    };

    Game_Player.prototype.scanItemLighting = function() {
        let note = '';
        let config = {id: 0};
    ...
    const setupPage = Game_Event.prototype.setupPage;
    const initialize$1 = Game_Event.prototype.initialize;
    ...
    Game_Event.prototype.setupPage = function() {
        setupPage.apply(this, arguments);
        this.setupLighting();
    };
    ...
    Game_Event.prototype.setupLighting = function() {
        ...
        this.page().list.forEach((comment) => {
            if (comment.code === 108 || comment.code === 408) 
                parseNotes(this.lightingParams, comment.parameters.join(''));
        });
    };
    ...
    class Game_Lighting {
        ...
        /* Global API */
        setPluginStatus(status) {
            this.lighting.setStatus(status);
        }
        ...
        setMapAmbient(color, time) {
            this.lighting.setMapAmbient(ColorManager.stringToHex(color), time);
        }

        /* Per-lighting API */
        setStatus(id, status) {
            this.lighting.setLightingState(id, status);
        }

        setShadow(id, status) {
            this.lighting.setShadowState(id, status);
        }

        setRadius(id, value, time, type) {
            this.lighting.setRadius(id, value, time, type);
        }

        setAngle(id, value, time, type) {
            this.lighting.setAngle(id, value, time, type);
        }

        setOffset(id, x, y, time, type) {
            this.lighting.setOffset(id, x, y, time, type);
        }

        setOffsetX(id, value, time, type) {
            this.lighting.setOffsetX(id, value, time, type);
        }

        setOffsetY(id, value, time, type) {
            this.lighting.setOffsetY(id, value, time, type);
        }

        setTint(id, value, time, type) {
            this.lighting.setOffsetY(id, value, time, type);
        }
    }
